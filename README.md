...
4. **[`CROWDING.md`](CROWDING.md), the mechanism test.** Does crowding in the short-volatility trade amplify its own unwind? Short volatility is modelled as a congestion game: ETPs must buy vega when volatility rises, constrained traders cover into the same move, and the loop amplifies shocks by `1/(1 − λK)`. Traders internalise their own tail loss but not their contribution to `λK`, so within the model the Nash equilibrium sits 1.18 to 1.77 above the social optimum. Four predictions were pre-registered and pushed here before the test code existed ([`CROWDING-PREREG.md`](CROWDING-PREREG.md)). Two are falsified on FINRA short interest and two pass only on the primary of three measures, so the chapter is a null. §6 carries a project-level multiplicity note, which no per-study deflated Sharpe in this repo accounts for.
