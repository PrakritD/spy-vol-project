...
The strategy's durable edge is drawdown control (Calmar 0.56 vs SPY 0.38); titles cite metrics
verbatim from strategy_results.json.
"""
...
a.set_title("Borrow sensitivity: the carry stays profitable and the\ndrawdown edge over SPY holds past 20%/yr borrow", fontsize=10.5)
...
axL.text(0.02, SPYx["sharpe"] + 0.012, f"SPY Sharpe {SPYx['sharpe']:.2f}", fontsize=8, color="#333")
axL.set_title("Construction ladder: the contango filter is the lever\nit doubles Calmar and halves drawdown; extra gates only cost carry", fontsize=11)
...
fig.suptitle("Research depth: the term-structure filter carries the edge; vol-targeting, gamma, and extra gates do not",
             fontsize=12, y=1.02)
fig.text(0.5, -0.04,
         f"Net of 10bps + 3%/yr borrow. Deflated Sharpe {R['dsr']['range'][0]:.2f}–{R['dsr']['range'][1]:.2f} over {R['dsr']['n_trials']} variants; "
         f"bootstrap P(SR≤0)={R['bootstrap_ci_vs0'][2]:.3f} vs zero. "
         "SPY-timing sleeve is a null (OOS AUC 0.51).",
